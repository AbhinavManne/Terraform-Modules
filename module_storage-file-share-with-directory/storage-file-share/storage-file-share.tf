...
resource "azurerm_storage_share_file" "example" {
  count = length(var.storage_share_file_name)
  name             = var.storage_share_file_name[count.index].name
  storage_share_id = azurerm_storage_share.fileshare[count.index].id
  source           = var.storage_share_file_source[count.index].source
}

#Creating a Directory within an Azure Storage File Share
resource "azurerm_storage_share_directory" "example" {
  count=length(var.storage_share_directory_name)
  name                 = var.storage_share_directory_name[count.index].name
  share_name           = azurerm_storage_share.fileshare[count.index].name
  storage_account_name = azurerm_storage_account.sa.name
}
