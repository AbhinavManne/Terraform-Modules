...
# <u> Introduction </u>
* This module provides the facility to provision Windows Virtual machine Scale Set. In this module we have included all possible argument for windows virtual machine scale set.
...
| Name | Type |
|------|------|
| [azurerm_windows_virtual_machine_scale_set.wvmss](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/windows_virtual_machine_scale_set) | resource |
...
| <a name="input_add_secret"></a> [add\_secret](#input\_add\_secret) | Map of secret which have keyvault id and a object of certificate.  | <pre>map(object({<br>    key_vault_id = string<br>    certificate = map(object({<br>      store = string<br>      url = string<br>    })) <br>  }))</pre> | `null` | no |
...
| <a name="input_capacity_reservation_group_id"></a> [capacity\_reservation\_group\_id](#input\_capacity\_reservation\_group\_id) | Specifies the ID of the Capacity Reservation Group which the Virtual Machine Scale Set should be allocated to. Changing this forces a new resource to be created. capacity\_reservation\_group\_id cannot be used with proximity\_placement\_group\_id. single\_placement\_group must be set to false when capacity\_reservation\_group\_id is specified. | `string` | `null` | no |
| <a name="input_certificate_url"></a> [certificate\_url](#input\_certificate\_url) | The Secret URL of a Key Vault Certificate, which must be specified when protocol is set to Https. | `string` | `null` | no |
| <a name="input_content_additional_unattend_content"></a> [content\_additional\_unattend\_content](#input\_content\_additional\_unattend\_content) | The XML formatted content that is added to the unattend.xml file for the specified path and component. Changing this forces a new resource to be created. | `string` | n/a | yes |
...
| <a name="input_data_disk_encryption_set_id"></a> [data\_disk\_encryption\_set\_id](#input\_data\_disk\_encryption\_set\_id) | The ID of the Disk Encryption Set which should be used to encrypt this Data Disk. Disk Encryption Sets are in Public Preview in a limited set of regions. | `string` | `null` | no |
