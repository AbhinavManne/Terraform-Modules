...
variable "generate_admin_ssh_key" {
  type        = bool
  description = "Wants to generate admin ssh key?"
}
...
variable "disable_password_authentication" {
  type        = string
  description = "true/false- disable_password_authentication "
}

variable "allow_extension_operations" {
  type        = string
  description = "Enable-allow_extension_operations "
}
...
variable "storage_account_type" {
  type        = string
  description = "Storage account type of OS disk"
}
...
#-------------------------------------RESOURCE: DATA DISK AND ATTACHMENT----------------------------------#
...
variable "before_shutdown_time_in_minutes" {
  type        = number
  description = "Time in minutes between 15 and 120 before a shutdown event at which a notification will be sent. Defaults to 30."
  validation {
    condition     = var.before_shutdown_time_in_minutes >=15 && var.before_shutdown_time_in_minutes <=120 
    error_message = "Time in minutes between 15 and 120 "
  }
}

variable "webhook_url" {
  type        = string
  description = "The webhook URL to which the notification will be sent. Required if enabled is true. Optional otherwise."
}
...
#----------------------------------RESOURCE: VIRTUAL MACHINE EXTENSION(DEPENDENCY AGENT FOR LINUX)---------#
variable "vm_dependency_agent_name" {
  type        = string
  description = "The name of the virtual machine extension peering. "
}

variable "vm_dependency_agent_publisher" {
  type        = string
  description = "The publisher of the extension, available publishers can be found by using the Azure CLI. "
}

variable "vm_dependency_agent_type" {
  type        = string
  description = "The type of extension, available types for a publisher can be found using the Azure CLI. "
}

variable "vm_dependency_agent_type_handler_version" {
  type        = string
  description = "Specifies the version of the extension to use, available versions can be found using the Azure CLI. "
}

variable "vm_dependency_auto_upgrade_minor_version" {
  type        = string
  description = "Enable auto upgrade minor version or not"
}
