...
#-----------------------------------------RESOURCE: VM EXTENSION------------------------------------------#
#Create a Virtual Machine Extension (a monitoring agent) (basically log analytics agent)
...
#-----------------------------------------RESOURCE: LOG ANALYTICS RESOURCES--------------------------------#
...
resource "azurerm_log_analytics_datasource_windows_event" "example" {
  name                = var.datasource_windows_event_name
  resource_group_name = data.azurerm_resource_group.rg.name
  workspace_name      = azurerm_log_analytics_workspace.example.name
  event_log_name      = var.datasource_windows_event_log_name
  event_types         = var.datasource_windows_event_types
}

resource "azurerm_log_analytics_datasource_windows_performance_counter" "example" {
  name                = var.datasource_windows_performance_name
  resource_group_name = data.azurerm_resource_group.rg.name
  workspace_name      = azurerm_log_analytics_workspace.example.name
  object_name         = var.datasource_windows_performance_object_name
  instance_name       = var.datasource_windows_performance_instance_name
  counter_name        = var.datasource_windows_performance_counter_name
  interval_seconds    = var.datasource_windows_performance_interval
}
